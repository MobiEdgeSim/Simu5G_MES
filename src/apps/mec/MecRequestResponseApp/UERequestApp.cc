...
void UERequestApp::initialize(int stage)
{
    ...
    cycleTimer_ = new cMessage("cycleTimer"); // tianhao
...
//    EV << "UERequestApp::initialize - binding to port: local:" << localPort_ << " , dest:" << deviceAppPort_ << endl;
    simtime_t ueCycleTime = par("ueCycleTime");//tianhao
    scheduleAt(simTime() + ueCycleTime, cycleTimer_);
...
void UERequestApp::handleMessage(cMessage *msg)
{
    ...
    if (msg->isSelfMessage())
    {
        if(!strcmp(msg->getName(), "selfStart"))
            ...
            sendRequest();
        else if (strcmp(msg->getName(), "cycleTimer") == 0)
        {
            EV << "UERequestApp::handleMessage - Cycle timer triggered, resetting UE app state for next task placement." << endl;
            ...
            scheduleAt(simTime(), selfStart_);

            simtime_t ueCycleTime = par("ueCycleTime");
            scheduleAt(simTime() + ueCycleTime, cycleTimer_);
        ...
        {
            throw cRuntimeError("UERequestApp::handleMessage - Unrecognized self message");
        }
    }
    // Receiver Side
    else
...
void UERequestApp::sendRequest()
{
    ...
    start_ = simTime();
    EV << "mecAppAddress_ : " << mecAppAddress_.str() << "    mecAppPort_ : " << mecAppPort_ << endl;
