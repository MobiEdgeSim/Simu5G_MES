        ...
        switch(appState)
        {
            case START:
            {
                ...
                else
                {
                    ...
                    EV<<"DeviceApp::handleUALCMPMessage() - this should not happen. This device app sends well formed requests to the UALCMP."<<endl;
            ...
            case CREATING:
            {
                ...
                if(response->getCode() == 201) // Successful response of the post
                {
                    ...
                    else
                    {
                        ...
                        if (endPoint[0] == "<unspec>") {
                            EV_ERROR << "DeviceApp::handleUALCMPMessage - ERROR MEC instance didn't define the ip address" << endl;
                            throw cRuntimeError("DeviceApp::handleUALCMPMessage - ERROR MEC instance didn't define the ip address");
                        }
            ...
            case IDLE:
            {
                EV_WARN << "DeviceApp::handleUALCMPMessage() - got a response in IDLE state. Possibly leftover or asynchronous. Ignoring.\n";

                break;
            }
            default:
                EV_WARN <<"DeviceApp::handleUALCMPMessage() - appstate IDLE. No messages should arrive from the UALCMP";
...
void DeviceApp::sendStartAppContext(inet::Ptr<const DeviceAppPacket> pk)
{
    ...
    //if(UALCMPSocket_.getState() == inet::TcpSocket::CONNECTED && appState == IDLE)//tianhao
...
//    EV << packet << endl;
    ...
    if(res)
    {
        ...
        if (processedUALCMPMessage->isScheduled())
                cancelEvent(processedUALCMPMessage);
