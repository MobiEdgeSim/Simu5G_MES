...
void GtpUser::handleMessage(cMessage *msg)
{
    ...
    else if (strcmp(msg->getArrivalGate()->getFullName(), "socketIn") == 0)
    {
        ...
        else if (inet::Indication *indication = dynamic_cast<inet::Indication *>(msg))
        {
            EV << "GtpUser::handleMessage - Received an Indication message: " << indication->getName() << endl;

            delete indication;
        }
        else
        {
            EV << "GtpUser::handleMessage - Received an unknown message type: " << msg->getClassName() << endl;
            delete msg;
        }
    }
}
