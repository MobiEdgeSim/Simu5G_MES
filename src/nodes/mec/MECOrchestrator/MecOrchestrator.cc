...
void MecOrchestrator::initialize(int stage) {
    Enter_Method("MEC Orchestrator initialize");
    ...
    else if (!strcmp(par("selectionPolicy"), "OptimalSelectionBased"))
        mecHostSelectionPolicy_ = new OptimalSelectionBased(this);
    else
    ...
    algorithmName = par("algorithmName").stringValue();
    getConnectedMecHosts();
    onboardApplicationPackages();

    //Scheduler a self-message to update mecHostList periodically
    updateMsg = new cMessage("updateMecHost");
    scheduleAt(simTime() + instantiationTime, updateMsg); //update every 0.1s
}

void MecOrchestrator::handleMessage(cMessage *msg) {
    Enter_Method("handle Message");
    if (msg->isSelfMessage()) {
        //handle the self-message to update the mecHostList
        if (msg == updateMsg) {
            updateMecHost();
            //updateMsg->drop();

            scheduleAt(simTime() + instantiationTime, updateMsg);
            return;
        }
...
void MecOrchestrator::startMECApp(UALCMPMessage *msg) {
    ...
    for (const auto &contextApp : meAppMap) //meAppMap is a map. can be found in the head file
    {
        /*
         * TODO
...
//        throw cRuntimeError("MecOrchestrator::startMECApp - Application package with AppDId[%s] not onboarded", contAppMsg->getAppDId());
    ...
    ApplicationDescriptor &desc = it->second;//changed for the ue ip setting

    //set the UE IP address
    desc.setUeIpAddress(contAppMsg->getUeIpAddress());
    EV << "MecOrchestrator::startMECApp - Set Ue IP address:"<< contAppMsg->getUeIpAddress()<< endl;
...
//        newMecApp.lastAckStartSeqNum = pkt->getSno();
        ...
        EV << "MecOrchestrator::startMECApp - new MEC application with name: " << appInfo->instanceId << " instantiated on MEC host []" << newMecApp.mecHost << " at " << appInfo->endPoint.addr.str()
...
//            throw cRuntimeError("MecOrchestrator::ackMEAppPacket - ERROR meApp[%d] does not exist!", contextId);
        ...
        EV << "Constructed referenceURI: " << uri.str() << endl;//TODO: TEST
...
/**
 * TODO
 * This methord is for the mobile mec environment.
 * when it found new mecHost =>addMecHost
 * delete the old mecHost =>addMecHost
 * list changed => updateMecHostListParam
 */
void MecOrchestrator::updateMecHost() {
    std::vector<cModule*> newMecHosts;
    cModule *network = getSimulation()->getSystemModule();

    // Find all current mecHost modules in the simulation
    for (cModule::SubmoduleIterator it(network); !it.end(); ++it) {
        cModule *submod = *it;
        if (strstr(submod->getName(), "mecHost") != nullptr) {
            newMecHosts.push_back(submod);
        }
    }

    // Check if there is any change in the mecHost list
    bool hasChanged = false;

    // Check for new MEC hosts to add
    for (auto &newHost : newMecHosts) {
        if (std::find(mecHosts.begin(), mecHosts.end(), newHost) == mecHosts.end()) {
            mecHosts.push_back(newHost);  // Simply add the new host to the list
            hasChanged = true;
        }
    }

    // Check for MEC hosts to remove
    for (auto it = mecHosts.begin(); it != mecHosts.end();) {
        if (std::find(newMecHosts.begin(), newMecHosts.end(), *it) == newMecHosts.end()) {
            it = mecHosts.erase(it);  // Remove the host from the list
            hasChanged = true;
        } else {
            ++it;
        }
    ...
    // Only update the mecHostList parameter if there was a change
    if (hasChanged) {
        updateMecHostListParam();
    }
}

void MecOrchestrator::updateMecHostListParam() {
    ...
    EV << "Updated mecHostList: " << newMecHostList << endl;
}

const std::vector<cModule*>& MecOrchestrator::getMecHosts() const
{
    return mecHosts;
}
