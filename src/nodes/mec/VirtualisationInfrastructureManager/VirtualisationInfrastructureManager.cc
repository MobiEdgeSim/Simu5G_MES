...
//    interfaceTableModule = par("interfaceTableModule").stringValue();
    ...
    interfaceTable = check_and_cast<inet::InterfaceTable*>(mecHost->getSubmodule("virtualisationInfrastructure")->getSubmodule("interfaceTable"));//TODO: !!!! upf_mec OR virtualisationInfrastructure
    ...
    {
        EV<<"VirtualisationInfrastructureManager::initialize - interface name: "<<interfaceTable->getInterface(i)->getInterfaceName()<<endl;
        ...
        else if(strcmp("cellular", interfaceTable->getInterface(i)->getInterfaceName()) == 0)//this is for the mobile mechost --Tianhao
        {
            mecAppRemoteAddress_ = interfaceTable->getInterface(i)->getIpv4Address();
        }
        EV << "mecAppRemoteAddress_: " << mecAppRemoteAddress_.str() << endl;
//        else
//        {
...
ResourceDescriptor VirtualisationInfrastructureManager::getMaxResources() const {
    ResourceDescriptor avRes;
    avRes.ram = maxRam;
    avRes.disk = maxDisk;
    avRes.cpu = maxCPU;
